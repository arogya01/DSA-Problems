...
class MeetingSchedulerApp {
    ...
    constructor() {
        ...
        this.meetingService = new MeetingService_1.MeetingService(4); // 4 worker threads
        this.notificationService = new NotificationService_1.NotificationService(2); // 2 worker threads
    }
    /**
    ...
    async initialize() {
        ...
        // Create some users
        const alice = await this.userService.createUser('Alice Smith', 'alice@example.com', 'America/New_York');
        this.users.set(alice.id, alice);
        const bob = await this.userService.createUser('Bob Johnson', 'bob@example.com', 'America/Los_Angeles');
        this.users.set(bob.id, bob);
        const charlie = await this.userService.createUser('Charlie Brown', 'charlie@example.com', 'Europe/London');
        this.users.set(charlie.id, charlie);
        const diana = await this.userService.createUser('Diana Prince', 'diana@example.com', 'Asia/Tokyo');
        ...
        try {
            const meeting = await this.meetingService.createMeeting('Project Kickoff', 'Discuss the new project goals and timeline', meetingStartTime, meetingEndTime, alice.id, participants, 8, // High priority
            Meeting_1.RecurrenceType.NONE);
            ...
            setTimeout(async () => {
                try {
                    const updatedMeeting = await this.meetingService.updateParticipantStatus(meeting.id, bob.id, 'ACCEPTED');
            ...
            setTimeout(async () => {
                try {
                    const updatedMeeting = await this.meetingService.updateParticipantStatus(meeting.id, diana.id, 'DECLINED');
            ...
            setTimeout(async () => {
                try {
                    const updatedMeeting = await this.meetingService.updateMeeting(meeting.id, 'Project Kickoff - Updated', 'Discuss the new project goals, timeline, and resource allocation', 'Conference Room A');
            ...
            setTimeout(async () => {
                try {
                    ...
                    const conflictingMeeting = await this.meetingService.createMeeting('Conflicting Meeting', 'This meeting conflicts with the existing one', meetingStartTime, meetingEndTime, alice.id, conflictingParticipants, 5, // Lower priority than the existing meeting
                    Meeting_1.RecurrenceType.NONE);
            ...
            setTimeout(async () => {
                try {
                    ...
                    const recurringMeeting = await this.meetingService.createMeeting('Weekly Team Sync', 'Regular team sync meeting', recurringStartTime, recurringEndTime, alice.id, recurringParticipants, 7, // High priority
                    Meeting_1.RecurrenceType.WEEKLY, recurrenceEndDate);
     ...
     */
    scheduleReminder(meeting, minutesBefore) {
        // Clear any existing reminder
        this.clearReminder(meeting.id);
        // Schedule a new reminder
        const reminder = this.notificationService.scheduleReminder(meeting, this.users, minutesBefore);
