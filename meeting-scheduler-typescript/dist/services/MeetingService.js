...
class MeetingService {
    ...
    constructor(numThreads = 4) {
        ...
        this.schedulingMutex = new concurrencyUtils_1.Mutex();
     ...
     */
    async createMeeting(title, description, startTime, endTime, organizer, participants, priority = 5, recurrenceType = Meeting_1.RecurrenceType.NONE, recurrenceEndDate, location) {
        // Use a mutex to ensure only one scheduling operation happens at a time
        return this.schedulingMutex.withLock(async () => {
            // Check for scheduling conflicts
            await this.checkForSchedulingConflicts(startTime, endTime, [organizer, ...participants.map(p => p.userId)], recurrenceType, recurrenceEndDate, priority);
            // Create the meeting
            const meeting = new Meeting_1.Meeting(title, description, startTime, endTime, organizer, participants, priority, recurrenceType, recurrenceEndDate, location);
            // Store the meeting
    ...
    async getMeetingsForUser(userId) {
        return this.lock.withReadLock(() => {
            return Array.from(this.meetings.values()).filter(meeting => (meeting.organizer === userId ||
                meeting.participants.some(p => p.userId === userId)) &&
                meeting.status !== Meeting_1.MeetingStatus.CANCELED);
        });
    }
    /**
     ...
     * @returns List of meetings
     */
    async getMeetingsForUserInTimeRange(userId, startTime, endTime) {
        return this.lock.withReadLock(() => {
            return Array.from(this.meetings.values()).filter(meeting => (meeting.organizer === userId ||
                meeting.participants.some(p => p.userId === userId)) &&
                meeting.status !== Meeting_1.MeetingStatus.CANCELED &&
                (0, timeUtils_1.doTimeRangesOverlap)(meeting.startTime, meeting.endTime, startTime, endTime));
        });
    }
    /**
     * Update a meeting
     ...
     */
    async updateMeeting(meetingId, title, description, location) {
     ...
     */
    async rescheduleMeeting(meetingId, newStartTime, newEndTime) {
        return this.schedulingMutex.withLock(async () => {
            ...
            await this.checkForSchedulingConflicts(newStartTime, newEndTime, participants, meeting.recurrenceType, meeting.recurrenceEndDate, meeting.priority, meetingId // Exclude this meeting from conflict check
            );
            // Update the meeting
            await this.lock.withWriteLock(() => {
    ...
    async addParticipant(meetingId, userId, isRequired = true) {
        return this.schedulingMutex.withLock(async () => {
            ...
            const userMeetings = await this.getMeetingsForUserInTimeRange(userId, meeting.startTime, meeting.endTime);
            if (userMeetings.length > 0) {
                // Check if there's a conflict
                const conflictingMeeting = userMeetings.find(m => m.priority >= meeting.priority);
                if (conflictingMeeting) {
                    throw new Error(`User has a conflicting meeting with higher or equal priority: ${conflictingMeeting.title}`);
                }
            }
            // Add the participant
            await this.lock.withWriteLock(() => {
     ...
     */
    async checkForSchedulingConflicts(startTime, endTime, participants, recurrenceType, recurrenceEndDate, priority = 5, excludeMeetingId) {
        // For recurring meetings, generate all occurrences
        let occurrences = [];
        if (recurrenceType !== Meeting_1.RecurrenceType.NONE && recurrenceEndDate) {
            occurrences = (0, timeUtils_1.generateRecurringMeetingDates)(startTime, endTime, recurrenceType, recurrenceEndDate);
        }
        else {
        ...
        for (const occurrence of occurrences) {
            ...
            const tasks = participants.map(userId => {
                return this.threadPool.executeTask((0, ThreadPool_1.createTask)('checkUserAvailability', async (userId, startTime, endTime, priority, excludeMeetingId) => {
                    // Get all meetings for the user in this time range
                    const userMeetings = await this.getMeetingsForUserInTimeRange(userId, startTime, endTime);
                    // Filter out the excluded meeting
                    const conflictingMeetings = userMeetings.filter(meeting => !excludeMeetingId || meeting.id !== excludeMeetingId);
                    // Check if there's a conflict with higher priority
                    const higherPriorityConflict = conflictingMeetings.find(meeting => meeting.priority > priority);
                ...
                }, userId, occurrence.start, occurrence.end, priority, excludeMeetingId));
            });
            ...
            if (conflicts.length > 0) {
                ...
                throw new Error(`Scheduling conflict: ${conflict.conflictingMeeting?.title || 'Unknown meeting'} has higher priority`);
            }
        }
    }
    /**
     ...
     * @param userId User ID
     */
    sendMeetingInvitation(meeting, userId) {
        this.threadPool.executeTask((0, ThreadPool_1.createTask)('sendMeetingInvitation', async (meetingId, userId) => {
    ...
    sendMeetingUpdates(meeting) {
        ...
        for (const userId of participants) {
            this.threadPool.executeTask((0, ThreadPool_1.createTask)('sendMeetingUpdate', async (meetingId, userId) => {
                // In a real implementation, this would send an email or other notification
    ...
    sendMeetingCancellation(meeting) {
        ...
        const participants = [meeting.organizer, ...meeting.participants.map(p => p.userId)];
        for (const userId of participants) {
            this.threadPool.executeTask((0, ThreadPool_1.createTask)('sendMeetingCancellation', async (meetingId, userId) => {
                // In a real implementation, this would send an email or other notification
