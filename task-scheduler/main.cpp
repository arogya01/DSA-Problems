...
public:
    int leastInterval(vector<char> &tasks, int n)
    {
        ...
        while (!pq.empty())
        {
            ...
            if (pq.empty())
                break;     // if the priority queue is empty than all tasks are completed and we don't need to include the idle time
...
int main()
{
    vector<char> sample{'A', 'A', 'B', 'B'};

    Solution s;
    cout << s.leastInterval(sample, 2);
}